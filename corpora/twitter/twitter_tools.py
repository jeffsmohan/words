...
def count_words(args: argparse.Namespace) -> None:
    ...
    line_count_output = subprocess.check_output(f'wc -l {args.input}', shell=True, encoding='utf-8')
    line_count = int(re.search(r'^\s*(\d+)', line_count_output).groups()[0])

    # Read tweets in from file and process their words
    word_counts = collections.Counter()
    with open(args.input, encoding='utf-8') as f, tqdm.tqdm(total=line_count, unit='tweet', unit_scale=True) as progress:
        tweets = []
        for i, tweet in enumerate(f):
            ...
            if i % PROCESS_CHUNK_SIZE == 0:
                ...
                word_counts = collections.Counter(dict(word_counts.most_common(MAX_WORD_COUNT_LENGTH)))
...
if __name__ == "__main__":
    ...
    parser.add_argument('-i', '--input', default='tweets.txt', help='Specifies the tweet dataset to ingest')
