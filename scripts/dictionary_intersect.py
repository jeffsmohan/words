...
def read_word_lists(paths: List[str], ignore_patterns: List[str]) -> List[set]:
    """Read word lists from disk and returns a list of sets."""
    word_lists = []
    for path in paths:
        word_list = set()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="append",
        required=True,
        help="Input text files of words to intersect (provide two or more)",
    )
    parser.add_argument(
        "--output",
        "-o",
    ...
    parser.add_argument(
        ...
        help="Regex patterns identifying lines to ignore from the word lists "
        "(e.g., `^#!comment:`)",
    )
